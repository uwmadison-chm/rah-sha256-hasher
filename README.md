...
This takes the SHA-256 hash of a value that arrives in a REDCap alert and writes the digest back into the record through the REDCap API. Hashing isn't something REDCap can do on its own, so it's a small but real example of the kind of work you'd hand off to rah -- an alert comes in, a handler does something REDCap can't, and a record gets updated.
...
* `redcap_info_file` (string) The path to a TOML file with the REDCap `url` and `token`
...
The `redcap_info_file` is kept out of the route config on purpose, so the API token never lands in the same file as everything else. It's a TOML file with two keys:
...
* Read `redcap_info_file` to get the API address and token
...
* Take `sha256(value)` -- UTF-8 bytes, lowercase hex
